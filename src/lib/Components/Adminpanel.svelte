...
<div class="flex w-full flex-col gap-5 px-2 lg:flex-row lg:px-0">
	<div class="w-full lg:w-1/2">
		...
		>
			...
			<button onclick={() => addSomething(30, user.id)}> Controle de Atividades atualizado </button>
			...
			<div class="flex flex-col items-center gap-5">
				<div class="flex flex-col items-center gap-1">
					<div class="flex flex-col items-center gap-2">
						<p>Esta ação irá zerar todos os pontos deste usuário(a)</p>
					</div>
				</div>
				<button
					onclick={() => {
						addSomething(-1000000, user.id), (dangerZone = false);
					}}
				...
				<button
					class="bg-primary/30 border-primary w-fit cursor-pointer rounded-lg border p-2 hover:font-bold hover:opacity-50"
					onclick={() => (dangerZone = false)}>Fechar</button
				>
			</div>
		</div>
	</div>
